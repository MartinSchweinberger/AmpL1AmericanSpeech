...
###############################################################
# save data to disc
write.table(wc, file = "datatables/wc.txt", sep = "\t", row.names = FALSE, col.names = TRUE, quote = F)
#wc <- read.table("datatables/wc.txt", sep = "\t", header = T, quote = "", comment.char = "")
###############################################################
# clean colnames
...
writeLines(hslldpos01, con = "datatables/hslldpos01.txt", sep = "\n", useBytes = FALSE)
###
hslldpos02 <- POStag(object = pos02)
hslldpos02 <- as.vector(unlist(hslldpos02))
writeLines(hslldpos02, con = "datatables/hslldpos02.txt", sep = "\n", useBytes = FALSE)
###
hslldpos03 <- POStag(object = pos03)
hslldpos03 <- as.vector(unlist(hslldpos03))
writeLines(hslldpos03, con = "datatables/hslldpos03.txt", sep = "\n", useBytes = FALSE)
###
hslldpos04 <- POStag(object = pos04)
hslldpos04 <- as.vector(unlist(hslldpos04))
writeLines(hslldpos04, con = "datatables/hslldpos04.txt", sep = "\n", useBytes = FALSE)
###
hslldpos05 <- POStag(object = pos05)
hslldpos05 <- as.vector(unlist(hslldpos05))
writeLines(hslldpos05, con = "datatables/hslldpos05.txt", sep = "\n", useBytes = FALSE)
###
hslldpos06 <- POStag(object = pos06)
hslldpos06 <- as.vector(unlist(hslldpos06))
writeLines(hslldpos06, con = "datatables/hslldpos06.txt", sep = "\n", useBytes = FALSE)
###
hslldpos07 <- POStag(object = pos07)
hslldpos07 <- as.vector(unlist(hslldpos07))
writeLines(hslldpos07, con = "datatables/hslldpos07.txt", sep = "\n", useBytes = FALSE)
###############################################################
# list pos tagged elements
postag.files = c("datatables/hslldpos01.txt", "datatables/hslldpos02.txt", 
                 "datatables/hslldpos03.txt", "datatables/hslldpos04.txt",
                 "datatables/hslldpos05.txt", "datatables/hslldpos06.txt", 
                 "datatables/hslldpos07.txt")
...
write.table(df5, "datatables/hslld_postagged.txt", sep = "\t", row.names = F, col.names = T)
# read data
#df5 <- read.table("datatables/hslld_postagged.txt", sep = "\t", header = T, quote = "", comment.char = "")
###############################################################
# clean data frame
...
write.table(df5, "datatables/hslld_postagged_2.txt", sep = "\t", row.names = F, col.names = T)
# read data
#df5 <- read.table("datatables/hslld_postagged_2.txt", sep = "\t", header = T, quote = "", comment.char = "")
###############################################################
# clean data frame
df5 <- apply(df5, 2, function(x){
  x <- str_replace_all(x, fixed("\""), "") } )
...
# clean data
biowc <- biowc %>%
  dplyr::rename(SpeakerId = id, Speaker = spk, Age = age, Gender = gender, 
                File = file, PID = pid, Name = name, Participants = participants,
                DateOfBirth = birth, Comment = comment, DateOfRecording = date, 
                Location = location, Visit = situation, SituationType = actvities,
                SocioeconomicStatus = ses, WordCount = wc) %>%
  dplyr::select(-language, -filespk, -all) %>%
  dplyr::mutate(Visit = str_replace_all(Visit, ";.*", "")) %>%
  dplyr::mutate(SituationType = str_replace_all(SituationType, "\\(", "qwertz")) %>%
  dplyr::mutate(SituationType = str_replace_all(SituationType, "qwertz.*", "")) %>%
  dplyr::mutate(ID = 1:nrow(biowc))
# remove column wctb$filespk
colnames(biowc)[15] <- "wctbfilespk"
biowc <- biowc %>%
  dplyr::select(-wctbfilespk)
# save results to disc
write.table(biowc, file = "datatables/biowc.txt", sep = "\t", row.names = FALSE, col.names = TRUE)
###############################################################
###                    SEARCH
###############################################################
# set parameters
...
int$file <- as.vector(unlist(sapply(int$file, function(x){
  ...
  x <- gsub("-", "/", x, fixed = T) } )))
##########################################################
# add meaningful column names data
int <- int %>%
  dplyr::mutate(PoS = str_replace_all(token, ".*/", "")) %>%
  dplyr::rename(File = file, PreceedingContext = pre, Token = token, 
                FollowingContext = post, EntirePreceedingContext = all.pre,
                Function = status, PreviousPoS = prepos, PreviousWord = prelex)
# extract all potential intensifiers
JJ <- int[int$PreviousPoS == "JJ",]
RB <- int[int$PreviousPoS == "RB",]
# combine tabels
test <- rbind(JJ, RB)
# extract vector of potential intensifiers
pints <- rownames(table(test$PreviousWord, test$PreviousPoS))
# show list of potential intensifiers
pints

# code intensifiers
int$Amplified <- as.vector(unlist(sapply(tolower(int$PreviousWord), function(x){
  x <- ifelse(x == "awful", 1,
    ifelse(x == "awfully", 1,
...
# clean data and cerate new columns
int <- int %>%
  dplyr::mutate(Visit = str_replace_all(File, "\\/.*", "")) %>%
  dplyr::mutate(Situation = str_replace_all(File, "HV[0-9]{1,1}\\/", "")) %>%
  dplyr::mutate(Situation = str_replace_all(Situation, "\\/.*", "")) %>%
  dplyr::mutate(Child = str_replace_all(File, ".*\\/", "")) %>%
  dplyr::mutate(Child = str_replace_all(tolower(Child), "[a-z]{2,2}[0-9]{1,1}", "")) %>%
  dplyr::mutate(TokenWord = str_replace_all(Token, "\\/.*", "")) %>%
  dplyr::mutate(TokenWord = str_trim(TokenWord, side = "both")) %>%
  dplyr::mutate(Function = ifelse(Function == "attr", "Arributive", "Predicative"))
# inspect data
head(int)

# test data
test <- int[int$Amplified == 1,]
p1 <- test$PreviousWord 
p2 <- test$TokenWord
p3 <- gsub("^ ", "", test$FollowingContext)
p3 <- gsub(" .*", "", p3)
p3 <- gsub("/.*", "", p3)
...
int <- int[int$TokenWord != "xxx",]
nrow(int)

int$rmv <- gsub("^ ", "", int$FollowingContext)
...
###############################################################
# save table to disc
write.table(int, file = "datatables/int.txt", sep = "\t", row.names = FALSE, col.names = TRUE)
###############################################################
###                   END of PART 1
###############################################################
